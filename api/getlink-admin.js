...
module.exports = async function (req, res) {
    ...
    try {
        ...
        if (pathname === '/api/getlink-admin/debug/sheet-config' && req.method === 'GET') {
            const configured = !!GETLINK_SHEET_APPS_SCRIPT_URL;
            const scriptProbe = configured
                ? await probeAppsScriptHealth(GETLINK_SHEET_APPS_SCRIPT_URL)
                : null;
            return res.status(200).json({
                success: true,
                configured,
                maskedUrl: configured ? maskAppsScriptUrl(GETLINK_SHEET_APPS_SCRIPT_URL) : '',
        ...
        if (pathname === '/api/getlink-admin/debug/sheet-scan' && req.method === 'GET') {
            ...
            return res.status(200).json({
                success: true,
                configured: !!GETLINK_SHEET_APPS_SCRIPT_URL,
                maskedUrl: GETLINK_SHEET_APPS_SCRIPT_URL ? maskAppsScriptUrl(GETLINK_SHEET_APPS_SCRIPT_URL) : '',
                startRow,
                limit,
