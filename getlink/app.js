...
function getFixModeConfig(mode = 'overload') {
    if (mode === 'household') {
        return {
            ...
            rule1: 'CHỈ sử dụng tính năng này khi tài khoản ĐANG BỊ LỖI HỘ GIA ĐÌNH dẫn đến không coi được',
            rule2: 'Nếu sử dụng khi thật sự KHÔNG BỊ LỖI HỘ GIA ĐÌNH sẽ dẫn đến bị TRỪ ĐI HẠN BẢO HÀNH',
    ...
    return {
        ...
        rule1: 'CHỈ sử dụng tính năng này khi tài khoản ĐANG BỊ QUÁ TẢI sử dụng dẫn đến không coi được',
        rule2: 'Nếu sử dụng khi thật sự KHÔNG BỊ QUÁ TẢI sẽ dẫn đến bị TRỪ ĐI HẠN BẢO HÀNH',
...
function normalizeOverloadFixErrorMessage(error) {
    ...
    const normalized = rawMessage.toLowerCase();
    if (normalized.includes('khong lay du 2 cookie pass tu google sheet')
        || normalized.includes('không lấy đủ 2 cookie pass từ google sheet')) {
        return `Không lấy đủ 2 cookie sống từ Google Sheet để tự ${config.actionLabel}. Vui lòng bấm CẦN HỖ TRỢ / BẢO HÀNH để được hỗ trợ.`;
    }
            ...
            const renderPending = (payload) => {
                setOverloadFixState(
                    buildGetlinkOperationText(
                        payload,
                        Date.now() - operationMeta.startedAt,
                        'Đang lấy 2 cookie mới từ Google Sheet rồi thay cookie chính...'
                    ),
                    'loading'
                );
            };
